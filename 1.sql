...
alter table vehicle add constraint fk_vehicle_cluster_path foreign key (cluster_path) references cluster (path) on delete restrict on update restrict;
create index ix_vehicle_cluster_path on vehicle (cluster_path);
...
alter table vehicle drop constraint if exists fk_vehicle_cluster_path;
drop index if exists ix_vehicle_cluster_path;
